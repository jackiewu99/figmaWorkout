...
export default function Workout2x2CreateForm(props) {
  const {
    clearOnSuccess = true,
    onSuccess,
    onError,
  ...
  const resetStateValues = () => {
    ...
    setRep(initialValues.rep);
    setArea(initialValues.area);
    setName2(initialValues.name2);
    setDemo2(initialValues.demo2);
    setRep2(initialValues.rep2);
    setErrors({});
  };
  const validations = {
    name: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          await DataStore.save(new Workout2x2(modelFields));
          if (onSuccess) {
      ...
      {...rest}
      {...getOverrideProps(overrides, "Workout2x2CreateForm")}
    >
      <TextField
        label="Name"
        isRequired={false}
